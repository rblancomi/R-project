...
Table 2: confidence parameter for each module transition \label{table2}
...
```
The fitness landscape in \autoref{fig:fig31} more clearly reflects how fitness increases with the accumulation of mutations in the order specified in the DAG of restrictions exponentially, as we set fitness parameter the same for every parent-child relationship. 
...
```{r fig32, fig.align="center", fig.cap= "Simulation of cancer progression using the fitness landscape of the simplied model for the COADREAD dataset (stacked plot)"}
...
```{r fig33, fig.align="center", fig.cap= "Simulation of cancer progression using the fitness landscape of the simplified model for the COADREAD dataset (line plot)"}
plot(COADREADsim_Simul, 
     show = "genotypes",
     type = "line"
)
```
```{r fig34, fig.align="center", fig.cap="Parent-child relationship derived from simulation"}
## Parent-child relationship derived from simulation
...
```
### \textcolor{blue}{Frequency-dependent fitness} {#four_one_three}
...
```{r fig35, fig.align="center", fig.cap="Fitness landscape corresponding with the first-five-nodes possible genotypes for the COADREAD dataset accounting for frequency-dependent fitness"}

# Mapping of genotypes to frequency-dependent fitness
COADREAD5_gen <- data.frame(Genotype = c("WT", "APC", "TP53", "KRAS",
                                         "APC, TP53", "APC, KRAS", "KRAS, TP53",
                                         "APC, TP53, KRAS",
                                         "APC, TP53, KRAS, PIK3CA",
                                         "APC, TP53, KRAS, NRAS",
                                         "APC, TP53, KRAS, LRP1B"),
                            Fitness = c(rep("1", 5), rep("0", 2), "1", 
...
```
```{r fig36, fig.align="center", fig.cap= "Simulation of cancer progression using the frequency-dependent fitness model for the COADREAD dataset (stacked plot)"}
...
```
```{r fig37, fig.align="center", fig.cap= "Simulation of cancer progression using the frequency-dependent fitness model for the COADREAD dataset (line plot)"}
plot(COADREAD5sim_Simul, 
     show = "genotypes",
     type = "line"
)
```
```{r fig38, fig.align="center", fig.cap="Parent-child relationship derived from simulation"}
## Parent-child relationship derived from simulation
plotClonePhylog(COADREAD5sim_Simul,
                N = 0, ## Specify clones that exist
                keepEvents = TRUE ## Arrows showing how many times each clones appeared
)
```

## \textcolor{blue}{Glioblastoma (GBM) dataset} {#four_two}

### \textcolor{blue}{Modelling mutual exclusivity and order restrictions} {#four_two_one}

Figure 8.C from @Neyshabouri2020 shows the CPM inferred from the COADREAD dataset, consisting of seven modules with between 1 to 3 genes each. The knowledge on GMB progression is not as extended as for colorectal cancer and there is not a clearly defined order of first driver mutations, but known driver events such as *EGFR* and *NF1* mutations are correctly inferred in the module @RefHere. Using the DAG of restrictions as starting point, the evolutionary model is created specifying satisfied and not satisfied genotype fitness for all modules following the same criterion as with the COADREAD dataset. The confidence parameter for each module transition is shown in \autoref{table:table3}. Again, dependency between modules (`typeDep`) is set to monotonic (MN) as it is a mandatory argument for `allFitnessEffects` function. 

Table 3: confidence parameter for each module transition \label{table3}
...
```{r fig39, fig.align="center", fig.cap= "DAG of restrictions for the GBM dataset"}
...
```
```{r fig40, fig.align="center", fig.cap= "Fitness landscape corresponding with the DAG of restrictions for the GBM dataset"}
# Evaluation of all possible genotypes fitness under the previous fitness specifications
...
```
### \textcolor{blue}{Simplified cancer progression model} {#four_two_two}
...
```{r fig41, fig.align="center", fig.cap= "Simplified DAG of restrictions for the GBM dataset"}
...
plot(GBMsim_fitness)
```
```{r fig42, fig.align="center", fig.cap= "Fitness landscape corresponding with the simplified DAG of restrictions for the GBM dataset"}
# Evaluation of all possible genotypes fitness under the previous fitness specifications
...
```
The fitness landscape in \autoref{fig:fig32} more clearly shows how fitness increases with the accumulation of mutations in the order specified in the DAG of restrictions exponentially.

Next, we use the simplified fitness landscape to simulate tumor progression for one individual with the `oncoSimulIndiv` functionality. 

```{r fig43, fig.align="center", fig.cap= "Simulation of cancer progression using the fitness landscape of the simplied model for the GBM dataset (stacked plot)"}
...
type = "stacked")
```
```{r fig44, fig.align="center", fig.cap= "Simulation of cancer progression using the fitness landscape of the simplified model for the GBM dataset (line plot)"}
plot(GBMsim_Simul, 
     show = "genotypes",
     type = "line"
)
```
```{r fig45, fig.align="center", fig.cap="Parent-child relationship derived from simulation"}
## Parent-child relationship derived from simulation
...
```
### \textcolor{blue}{Frequency-dependent fitness} {#four_two_three}
...
```{r fig46, fig.align="center", fig.cap="Fitness landscape corresponding with the first-two-nodes possible genotypes for the COADREAD dataset accounting for frequency-dependent fitness"}
...
# Fitness landscape representation
plotFitnessLandscape(GBM2_FL, use_ggrepel = TRUE) 
```
```{r fig47, fig.align="center", fig.cap= "Simulation of cancer progression using the frequency-dependent fitness model for the GBM dataset (stacked plot)"}
...
type = "stacked")
```
```{r fig48, fig.align="center", fig.cap= "Simulation of cancer progression using the frequency-dependent fitness model for the GBM dataset (line plot)"}
plot(GBM2sim_Simul, 
     show = "genotypes",
     type = "line"
)
```
```{r fig49, fig.align="center", fig.cap="Parent-child relationship derived from simulation"}
## Parent-child relationship derived from simulation
